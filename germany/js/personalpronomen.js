      ...
      class AdaptiveLearningGame {
        ...
        async loadData() {
          try {
            const savedWords = localStorage.getItem("person_words");
            if (savedWords) {
              this.words = JSON.parse(savedWords);
            } else {
              const wordsResponse = await fetch("json/personalpronomen.json");
        ...
        loadAllStates() {
          ...
          combinations.forEach(key => {
            const savedState = localStorage.getItem(`perso_state_${key}`);
            if (savedState) {
        ...
        saveData() {
          // Save words data
          localStorage.setItem("person_words", JSON.stringify(this.words));
          
          // Save current combination state
          const key = this.getCurrentKey();
          const currentState = this.getCurrentState();
          localStorage.setItem(`perso_state_${key}`, JSON.stringify(currentState));
          ...
          if (this.currentMode === "hard") {
            ...
            switch (this.currentQuestionType.type) {
              ...
              case "word_with_sentence":
                ...
                questionType.textContent = "Finde das personalpronomen";
        ...
        renderHardQuestion() {
          ...
          if (this.currentQuestionType.type === "word_with_sentence" || this.currentQuestionType.type === "sentence_only") {
            sentenceDisplay.textContent = `"${this.currentSentence.fa}"`;
            sentenceDisplay.classList.remove("hidden");
            questionType.textContent = "Schreibe das passende personalpronomen";
            wordDisplay.textContent = "?";
          } else {
            wordDisplay.textContent = this.currentWord.meaning;
        ...
        submitHardAnswer() {
          ...
          console.log(`🔍 DEBUG submitHardAnswer - ${this.getCurrentKey()}:`, {
            ...
            storageKey: `perso_state_${this.getCurrentKey()}`,
        ...
        resetProgress() {
          if (confirm("Are you sure you want to reset all progress for current combination?")) {
            const key = this.getCurrentKey();
            localStorage.removeItem(`perso_state_${key}`);

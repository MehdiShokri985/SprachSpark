...
let stored = JSON.parse(localStorage.getItem("selectedLevel"));
let level = stored ? JSON.parse(stored) : null;

if (level) {
  console.log(level.gruppe);
} else {
  console.log("هیچ سطحی انتخاب نشده است");
}
...
window.addEventListener("load", () => {
  ...
  fetch("../json/Gespräch/json-Gesprach-Gruppe.json")
    ...
    .then((levelConfig) => {
      // console.log(levelConfig.Gruppen[level.gruppe - 1].level);
      const levelGesprach = levelConfig.Gruppen[level.gruppe - 1].level;
      const thema_ = levelConfig.Gruppen[level.gruppe - 1].Themen[level.Nr - 1];
      if (thema_) {
        const config = thema_;
    ...
    .catch((err) => {
      container.innerHTML = `<div class="error">خطا در بارگذاری levelConfig: ${err.message}</div>`;
    });
});

function createFlashcard(item, index) {
  const mainItem = item;
  const mainSoundDe = (item.Sound_de || "").trim();
  const isMainSentence = /[.!?]$/.test(mainSoundDe);
  const personClass = item.person === "Person_A" ? "person-a" : "person-b";

  const flashcardDiv = document.createElement("div");
  flashcardDiv.className = `flashcard ${personClass}`;
    ...
    <div class="flashcard-inner">
      <div class="flashcard-front">
        <div class="item-top">
          ...
          <div class="person">${mainItem.person}</div>
        </div>
      ...
      <div class="flashcard-back">
        ...
        <div class="item-bottom">
          <div class="sound ${isMainSentence ? "sentence" : ""}"></div>
          <div class="ctrl-bottom">
            <audio src="${container.dataset.audioPath}/${
    mainItem.file || ""
  }" preload="none"></audio>
  ...
  const updateSliderBg = (val, max) => {
    ...
    slider.style.background = `linear-gradient(to right, #00ff88 ${percent}%, #2f547f ${percent}%)`;
  };
  ...
  flashcardDiv.addEventListener("click", (ev) => {
    if (ev.target.closest(".root-icon, .play-btn, .reveal-slider, .sound"))
      return;
    flashcardDiv.classList.toggle("flipped");
    const slider = flashcardDiv.querySelector(".reveal-slider");
    const spans = flashcardDiv.querySelectorAll(".sound span");
    if (slider && spans) {
      slider.value = 0;
...
function renderFlashcards(items) {
  ...
  items.forEach((item, index) => {
    const flashcard = createFlashcard(item, index);

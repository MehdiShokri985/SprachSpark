...
function createItem(group) {
    ...
    let typeHtml = "";
    if (mainItem.type && typeof mainItem.type === "string" && mainItem.type.trim() !== "") {
        ...
        if (isSentence) {
            ...
            soundContent = segments
                .map((word, index) => {
                    ...
                    const checkMultiWord = (arr, index) => {
                        ...
                        for (let item of arr) {
                            ...
                            if (phrase === item.toLowerCase()) {
                                return { match: true, length: words.length, phraseWords: segments.slice(index, index + words.length) };
                            }
                        }
                    ...
                    if (item.auxiliary_verb && item.auxiliary_verb.some(a => a && typeof a === "string" && a.toLowerCase() === cleanWord)) {
                        className = "aux-verb";
                    } else if (item.subject && item.subject.some(s => s && typeof s === "string" && s.toLowerCase() === cleanWord)) {
                        className = "subject";
                    } else if (item.verb && item.verb.some(v => v && typeof v === "string" && v.toLowerCase() === cleanWord)) {
                        className = "verb";
                    } else if (item.verb_part1 && item.verb_part1.some(vp1 => vp1 && typeof vp1 === "string" && vp1.toLowerCase() === cleanWord)) {
                        className = "verb_part1";
                    } else if (item.verb_part2 && item.verb_part2.some(vp2 => vp2 && typeof vp2 === "string" && vp2.toLowerCase() === cleanWord)) {
                        className = "verb_part2";
                    } else if (item.object && item.object.some(o => o && typeof o === "string" && o.toLowerCase() === cleanWord)) {
                        className = "object";
                    }
    ...
    const mainItemBottom = createItemBottom(mainItem, isMainSentence, document.querySelector('.content').dataset.audioPath);
    ...
    relatedItems.forEach(relatedItem => {
        ...
        const relatedItemBottom = createItemBottom(relatedItem, isRelatedSentence, document.querySelector('.content').dataset.audioPath);
...
function renderItems(items) {
    ...
    for (let i = 0; i < groups; i++) {
        ...
        accordionHeader.addEventListener("click", (e) => {
            if (e.target.classList.contains("toggle-textbox-btn") || e.target.closest(".toggle-textbox-btn") || e.target.classList.contains("test-btn") || e.target.closest(".test-btn")) {
                return;
            }
            ...
            toggleTextboxButton.disabled = !accordionContent.classList.contains("active");
            ...
            if (isActive) {
                ...
                const currentGroupItems = groupedItems.filter(group =>
                    group.some(item => parseInt(item.Filename) >= groupStart + 1 && parseInt(item.Filename) <= groupEnd)
                );
...
levelButtons.forEach(button => {
    button.addEventListener("click", () => {
        ...
        const jsonFile = level === "A1" ? "json-worterA1.json" : "json-worterA2.json";
        const audioPath = level === "A1" ? "audio_A1" : "audio_A2";
        document.querySelector("h1").textContent = `GOETHE-ZERTIFIKAT ${level} - WORTLISTE`;
